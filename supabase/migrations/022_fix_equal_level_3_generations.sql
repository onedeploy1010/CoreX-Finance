-- Fix: Equal-level bonus should only cascade up to 3 generations (was 5)
-- Also update team bonus formula description for clarity

-- 1. Update process_team_bonuses_daily: change equal-level max from 5 to 3
...
BEGIN
  ...
  LOOP
    ...
    IF v_team_reward > 0 THEN
      ...
      -- Cascading equal-level bonus: walk up chain, up to 3 same-level ancestors (was 5)
...
-- 2. Clean up any existing equal-level bonus records beyond generation 3

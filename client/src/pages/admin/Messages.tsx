...
export default function AdminMessages() {
  ...
  const { data: messageList = [], isLoading } = useQuery({ queryKey: ["/api/admin/messages"] });

  const createMutation = useMutation({
    mutationFn: async (data: any) => { await apiRequest("POST", "/api/admin/messages", data); },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, ...data }: any) => { await apiRequest("PATCH", `/api/admin/messages/${id}`, data); },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/messages"] });
      setEditing(null);
      toast({ title: "消息已更新" });
